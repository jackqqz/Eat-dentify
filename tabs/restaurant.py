...
def display_restaurant():
    """
    Display AI-suggested restaurants in an interactive expandable format.
    
    Shows restaurant search results with comprehensive information including:
    - Restaurant cards with expandable details with AI reasoning and navigation links for each result
    - Star rating display based on restaurant ratings
    - Interactive "Add Column" feature to query additional details about restaurants
...
def format_citation(citation):
    """
    Format citation text generated by AI for better readability in the UI.
    
    Args:
        citation (str): Raw citation text with Streamlit color background markers
        
    Returns:
        str: Formatted citation with proper line breaks and spacing
        
    Processes citation text by:
    - Adding line breaks before red and green background markers
    - Adding spacing after closing brackets
    - Improving visual separation of different citation elements
    
    This ensures citations are displayed in a clean, readable format
    within the restaurant information cards.
    """
    return citation.replace(':red-background[', '\n\n:red-background[').replace(':green-background[', '\n\n:green-background[').replace(']', '] \n\n \n\n')
